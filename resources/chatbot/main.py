...
REGION_SERVICES_URL = "https://api.regional-table.region-services.aws.a2z.com/index.json"

@app.post("/available-services")
async def available_services_api(region_name: str = Body(..., embed=True)):
    try:
        async with httpx.AsyncClient() as client:
            response = await client.get(REGION_SERVICES_URL)
            data = response.json()
        
        available_services = set()
        for item in data["prices"]:
            service_name = item.get("attributes", {}).get("aws:serviceName")
            region = item.get("attributes", {}).get("aws:region")
            
            if service_name and region and region.lower() == region_name.lower():
                available_services.add(service_name)
        
        result = ", ".join(sorted(available_services))
        result += f"\nTotal {len(available_services)} available."

        return {"services": result}
    except Exception as e:
        raise HTTPException(status_code=400, detail=f"Error fetching services: {str(e)}")
